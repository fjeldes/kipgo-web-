...
function AdminSidebar() {
  ...
  return (
    <>
      <button className="lg:hidden fixed top-4 left-4 z-50 p-2.5 rounded-xl bg-white shadow-[0px_20px_40px_rgba(26,35,126,0.06)]" onClick={() => setOpen(!open)}>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-white transform transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-16 flex items-center gap-2 px-6" style={{ borderBottom: '1px solid oklch(0 0 0 / 6%)' }}>
          <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-gradient-to-br from-[#000666] to-[#1a237e]">
            <span className="text-white font-bold text-sm">SC</span>
          </div>
          <span className="font-heading font-bold" style={{ color: '#1a1c1c' }}>Admin</span>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4" style={{ borderTop: '1px solid oklch(0 0 0 / 6%)' }}>

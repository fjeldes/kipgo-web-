...
import { ArrowLeft, MapPin, Package, DollarSign, CheckCircle, XCircle } from "lucide-react";
...
export default function StoreDetail() {
  ...
  if (!store) {
    return (
      <div className="text-center py-16">
        ...
        <Link href="/super/bookings" className="text-sm mt-2 inline-block hover:underline" style={{ color: '#000666' }}>{t('store.back_to_directory', lang)}</Link>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Breadcrumb */}
      <nav className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest mb-6" style={{ color: '#454652' }}>
        <Link href="/super/bookings" className="hover:underline" style={{ color: '#000666' }}>
          <ArrowLeft size={14} className="inline mr-1" />
          {t('store.back_to_directory', lang)}
        </Link>
      </nav>
        ...
        <div className="bg-white p-6 rounded-xl shadow-[0px_10px_20px_rgba(26,35,126,0.02)]">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2 rounded-lg" style={{ background: '#e0e0ff' }}>
              <Package size={18} style={{ color: '#000666' }} />
            </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-[0px_10px_20px_rgba(26,35,126,0.02)]">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2 rounded-lg" style={{ background: '#ffdbcb' }}>
              <XCircle size={18} style={{ color: '#9f4200' }} />
            </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-[0px_10px_20px_rgba(26,35,126,0.02)]">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2 rounded-lg" style={{ background: '#d9e2ff' }}>
              <CheckCircle size={18} style={{ color: '#001944' }} />
            </div>
              ...
              return (
                <div key={size}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-bold" style={{ color: '#1a1c1c' }}>{t(size === 'small' ? 'store.items_small' : size === 'medium' ? 'store.items_medium' : 'store.items_large', lang)}</span>
                    <span className="text-xs font-medium" style={{ color: '#454652' }}>
                      {occ} / {cap} {t('common.occupied', lang)}
                    </span>
                  </div>
        ...
        <div className="p-4 sm:p-8 border-b" style={{ borderColor: '#f3f3f3' }}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: '#454652' }}>{t('store.reviews', lang)}</p>
            </div>
            {allReviews.length > 0 && (
              <div className="flex items-center gap-2">
                <StarRating rating={Math.round(allReviews.reduce((s, r) => s + r.rating, 0) / allReviews.length)} />
                <span className="text-sm font-bold" style={{ color: '#1a1c1c' }}>{(allReviews.reduce((s, r) => s + r.rating, 0) / allReviews.length).toFixed(1)}</span>

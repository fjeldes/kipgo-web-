  ...
  return (
    <div>
      ...
      <div className="flex items-start gap-6 mb-10">
        <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-3xl flex-shrink-0" style={{ background: 'rgba(0,6,102,0.05)' }}>🏪</div>
        <div className="flex-1">
          {editing ? (
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
              className="text-4xl font-headline font-extrabold tracking-tight w-full bg-[#f3f3f3] border-none rounded-xl py-2 px-4 outline-none" style={{ color: '#000666' }} />
          ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
        ...
        <div className="bg-white p-6 rounded-xl shadow-[0px_10px_20px_rgba(26,35,126,0.02)]">
          ...
          <p className="text-3xl font-headline font-extrabold" style={{ color: '#000666' }}>{store.currency || 'USD'} {store.pricePerDay?.small || 0}</p>
            ...
            <tbody className="divide-y" style={{ borderColor: '#f3f3f3' }}>
              {sizes.map((size) => (
                <tr key={size} className="hover:bg-[#f3f3f3]/30 transition-colors">
                  <td className="px-8 py-5 font-semibold" style={{ color: '#1a1c1c' }}>{sizeLabels[size]}</td>
